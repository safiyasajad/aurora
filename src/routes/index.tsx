...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Direct access to premium natural Sri Lankan sapphires. NGJA registered. Buyer concierge, mine visits and custom jewelry from Beruwala, Colombo & Bangkok." },
      { property: "og:title", content: "Aurora Gems — Finest Natural Sri Lankan Sapphires" },
      { property: "og:description", content: "From mine to market: a complete buying experience for international gemstone buyers." },
...
const WHATSAPP = "https://wa.me/94774579076?text=Hello%20Aurora%20Gems%2C%20I%27d%20like%20to%20enquire";
      ...
      <div className="mx-auto max-w-7xl px-5 md:px-10 h-16 md:h-20 flex items-center justify-between">
        <a href="#top" className="flex items-center gap-3 group">
          ...
          <div className="leading-none">
            ...
            <div className="text-[9px] tracking-[0.3em] uppercase text-muted-foreground mt-0.5">Ceylon · Est. Trust</div>
          </div>
        </a>
        <nav className="hidden lg:flex items-center gap-9">
          {links.map(([l, h]) => (
            <a key={h} href={h} className="text-[11px] tracking-[0.22em] uppercase text-foreground/80 hover:text-gold transition-colors">
              {l}
            </a>
          ))}
        </nav>
        <div className="hidden lg:block">
          <a href="#contact" className="btn-gold">Book a Trip</a>
        </div>
        <button onClick={() => setOpen(!open)} aria-label="Menu" className="lg:hidden text-gold p-2">
      ...
      {open && (
        <div className="lg:hidden border-t border-border/40 bg-background/95 backdrop-blur-xl">
          <div className="px-5 py-6 flex flex-col gap-5">
            {links.map(([l, h]) => (
              <a key={h} href={h} onClick={() => setOpen(false)} className="text-sm tracking-[0.22em] uppercase text-foreground/80">{l}</a>
            ))}
            <a href="#contact" onClick={() => setOpen(false)} className="btn-gold w-full">Book a Trip</a>
          </div>
        </div>
      )}
    </header>
    ...
    <section id="top" className="relative min-h-[100svh] flex items-end pt-24 pb-16 md:pb-28">
      ...
      <div className="relative mx-auto max-w-7xl px-5 md:px-10 w-full">
        <div className="max-w-2xl animate-reveal">
          <span className="eyebrow">Ceylon Sapphires · Since Generations</span>
          ...
          <p className="mt-7 max-w-xl text-base md:text-lg text-foreground/75 leading-relaxed">
            Direct access to premium gemstones from Sri Lanka's most trusted sourcing networks. From mine to market, a complete buying experience for international gemstone buyers.
          </p>
          <div className="mt-10 flex flex-wrap gap-4">
            <a href="#contact" className="btn-gold">Book a Buying Trip</a>
            <a href="#collection" className="btn-ghost-gold">View Gem Collection</a>
          </div>
  ...
  return (
    <section className="border-y border-border/50 bg-ink/40">
      <div className="mx-auto max-w-7xl px-5 md:px-10 py-6 md:py-7 flex flex-wrap items-center justify-center gap-x-10 gap-y-3">
        {items.map((t, i) => (
          <div key={t} className="flex items-center gap-3 text-[10px] md:text-[11px] tracking-[0.25em] uppercase text-foreground/70">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="text-gold" strokeWidth="1.5"><path d="M20 6L9 17l-5-5"/></svg>
            <span>{t}</span>
...
function WhyUs() {
  const reasons = [
    { t: "Direct From Source", d: "Native Sri Lankan network with generations of sourcing relationships across Beruwala and Ratnapura." },
    { t: "Airport Concierge", d: "Personal pickup at Bandaranaike International and seamless transfer to your accommodation." },
    { t: "Curated Accommodation", d: "Vetted boutique stays in Beruwala arranged for your visit and trading schedule." },
    { t: "Two Trading Capitals", d: "Active presence in Sri Lanka and at the Jewelry Trade Center, Bangkok." },
    { t: "Certified Stones", d: "Independently certified gemstones with full export documentation handled on your behalf." },
    { t: "Investor Discretion", d: "Confidential, white-glove service for collectors, jewelers and wholesale buyers." },
  ];
  return (
    <section className="py-24 md:py-36">
      <div className="mx-auto max-w-7xl px-5 md:px-10">
        <SectionHeader
          ...
          title={<>Trusted by buyers <span className="italic text-gold-gradient">across continents</span></>}
        ...
        <div className="mt-16 grid gap-px bg-border/60 md:grid-cols-2 lg:grid-cols-3 border border-border/60">
          {reasons.map((r, i) => (
            <div key={r.t} className="group bg-background p-8 lg:p-10 hover:bg-card transition-colors duration-500">
...
function Collection() {
  const gems = [
    { name: "Royal Blue Sapphire", origin: "Ceylon · Heated/Unheated", img: gemRoyalBlue },
    { name: "Cornflower Blue Sapphire", origin: "Ceylon · Premium", img: gemCornflower },
    { name: "Padparadscha Sapphire", origin: "Rare · Pinkish-Orange", img: gemPadparadscha },
    { name: "Yellow Sapphire", origin: "Vivid Canary", img: gemYellow },
    { name: "Pink Sapphire", origin: "Ceylon · Hot Pink", img: gemPink },
    { name: "Star Sapphire", origin: "Six-Ray Asterism", img: gemStar },
  ];
  return (
    <section id="collection" className="py-24 md:py-36 bg-ink/40 border-y border-border/50">
      <div className="mx-auto max-w-7xl px-5 md:px-10">
        <SectionHeader
          ...
          title={<>A gallery of <span className="italic text-gold-gradient">Ceylon's rarest</span></>}
        ...
        <div className="mt-16 grid gap-6 md:gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {gems.map((g) => (
            <article key={g.name} className="group relative overflow-hidden border border-border/60 bg-card">
              ...
              <div className="absolute bottom-0 inset-x-0 p-6 lg:p-7">
                <div className="text-[10px] tracking-[0.3em] uppercase text-gold/80">{g.origin}</div>
                <h3 className="mt-2 text-2xl text-foreground">{g.name}</h3>
                <a href="#contact" className="mt-4 inline-flex items-center gap-2 text-[11px] tracking-[0.25em] uppercase text-gold-soft hover:text-gold transition-colors">
                  Enquire
  ...
  const cats = [
    {
      ...
      items: ["Natural Blue Sapphires", "Pink & Yellow Sapphires", "Padparadscha & Star Sapphires", "Rare Collector Stones", "Certified Gemstones", "Wholesale Parcels"],
    },
    {
      tag: "02 · Buyer Concierge",
      title: "Visiting Sri Lanka",
      items: ["Airport Pickup", "Beruwala Accommodation", "Personal Buying Assistance", "Gem Market Visits", "Mine Visit Arrangements", "Export Documentation"],
    },
    {
      tag: "03 · Custom Jewelry",
      title: "Bespoke Manufacturing",
      items: ["Engagement Rings", "Luxury Jewelry", "Bespoke Designs", "Gemstone Setting", "CAD Jewelry Design", "Master Craftsmanship"],
    },
  ];
  return (
    <section id="services" className="py-24 md:py-36">
      <div className="mx-auto max-w-7xl px-5 md:px-10">
        <SectionHeader
          eyebrow="Services"
          title={<>Every step, <span className="italic text-gold-gradient">white-glove</span></>}
              ...
              <ul className="mt-6 space-y-3">
                {c.items.map((i) => (
                  <li key={i} className="flex items-start gap-3 text-sm text-foreground/85">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-gold mt-1.5 shrink-0"><path d="M5 12l5 5L20 7"/></svg>
                    {i}
                  </li>
                ))}
              </ul>
...
function Experience() {
  const steps = [
    { n: "I", t: "Arrival", d: "We meet you at Bandaranaike International with private transfer to Beruwala or Colombo.", img: expAccommodation },
    { n: "II", t: "Sourcing", d: "Curated gem market visits, vetted dealer introductions and access to private inventories.", img: expSelection },
    { n: "III", t: "Mine & Origin", d: "Optional visits to working sapphire mines and traditional gem cutting houses.", img: expMine },
  ];
  return (
    <section id="experience" className="py-24 md:py-36 bg-ink/40 border-y border-border/50">
      <div className="mx-auto max-w-7xl px-5 md:px-10">
        <SectionHeader
          ...
          title={<>From arrival <span className="italic text-gold-gradient">to export</span></>}
          intro="A choreographed journey crafted around your time, your budget and your collecting goals."
        />
        <div className="mt-16 space-y-16 md:space-y-24">
          {steps.map((s, i) => (
            <div key={s.t} className={`grid md:grid-cols-12 gap-8 md:gap-14 items-center ${i % 2 ? "md:[&>div:first-child]:order-2" : ""}`}>
              <div className="md:col-span-7 relative">
                <div className="relative overflow-hidden border border-border/60">
                  <img src={s.img} alt={s.t} width={1200} height={900} loading="lazy" className="w-full aspect-[4/3] object-cover" />
                  <div className="absolute top-4 left-4 text-gold font-display text-5xl drop-shadow-lg">{s.n}</div>
                </div>
              </div>
              <div className="md:col-span-5">
                <div className="text-[10px] tracking-[0.3em] uppercase text-gold/70">Step {s.n}</div>
    ...
    <section id="jewelry" className="py-24 md:py-36">
      <div className="mx-auto max-w-7xl px-5 md:px-10 grid md:grid-cols-2 gap-12 md:gap-20 items-center">
        <div className="relative">
          <img src={jewelryRing} alt="Bespoke sapphire and diamond ring" width={1200} height={900} loading="lazy" className="w-full aspect-square object-cover border border-border/60" />
        ...
        <div>
          ...
          <p className="mt-6 text-foreground/75 leading-relaxed max-w-md">
            Our in-house atelier and CAD studio transform certified gemstones into heirloom engagement rings and high jewelry. Every commission is a private collaboration between collector and craftsman.
          </p>
          <ul className="mt-8 grid grid-cols-2 gap-3 text-sm text-foreground/80">
            {["Engagement Rings", "Luxury Jewelry", "Bespoke Designs", "Gemstone Setting", "CAD Design", "Heirloom Restoration"].map((x) => (
              <li key={x} className="flex items-center gap-2"><span className="w-1 h-1 rounded-full bg-gold"/>{x}</li>
            ))}
          </ul>
          <div className="mt-10 flex gap-4">
            <a href="#contact" className="btn-gold">Commission a Piece</a>
          </div>
        </div>
      </div>
    </section>
...
function Testimonials() {
  const quotes = [
    { q: "Rashard's network gave us access to stones we would never have seen on the open market. The Beruwala trip was flawless from arrival to export.", a: "Daniel R.", r: "Wholesale Buyer · New York" },
    { q: "A genuine, knowledgeable partner. Three Padparadscha sapphires sourced and certified for our private collection in under a week.", a: "Mei L.", r: "Collector · Hong Kong" },
    { q: "From airport pickup to the JTC office in Bangkok, every detail was handled. This is how high-end gemstone trading should feel.", a: "Andreas K.", r: "Jeweler · Antwerp" },
  ];
  return (
    <section className="py-24 md:py-36 bg-ink/40 border-y border-border/50">
      <div className="mx-auto max-w-7xl px-5 md:px-10">
        <SectionHeader
          ...
          title={<>Voices from <span className="italic text-gold-gradient">our buyers</span></>}
        />
        <div className="mt-14 grid md:grid-cols-3 gap-6 md:gap-8">
          {quotes.map((t, i) => (
            <figure key={i} className="border border-border/60 bg-background p-8 lg:p-10 relative">
              <svg viewBox="0 0 24 24" className="w-8 h-8 text-gold/40" fill="currentColor"><path d="M9 7H5a2 2 0 00-2 2v4a2 2 0 002 2h2v3H4v2h5v-7a4 4 0 00-4-4V9h4V7zm10 0h-4a2 2 0 00-2 2v4a2 2 0 002 2h2v3h-3v2h5v-7a4 4 0 00-4-4V9h4V7z"/></svg>
              ...
              <figcaption className="mt-8 pt-6 border-t border-border/60">
                ...
                <div className="text-[10px] tracking-[0.25em] uppercase text-muted-foreground mt-1">{t.r}</div>
              </figcaption>
            </figure>
          ))}
        </div>
...
function Contact() {
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const msg = `Hello Aurora Gems,%0A%0AName: ${encodeURIComponent(String(fd.get("name") || ""))}%0AEmail: ${encodeURIComponent(String(fd.get("email") || ""))}%0AArrival: ${encodeURIComponent(String(fd.get("date") || ""))}%0AInterest: ${encodeURIComponent(String(fd.get("interest") || ""))}%0A%0A${encodeURIComponent(String(fd.get("message") || ""))}`;
    ...
    <section id="contact" className="py-24 md:py-36">
      <div className="mx-auto max-w-7xl px-5 md:px-10 grid lg:grid-cols-12 gap-12 lg:gap-16">
        <div className="lg:col-span-5">
          ...
          <p className="mt-6 text-foreground/75 leading-relaxed max-w-md">
            Reach us directly via WhatsApp for the fastest response, or send a detailed inquiry through the form.
          </p>
          ...
          <div className="mt-10 space-y-5">
            ...
            <ContactRow label="Instagram" value="@Rashard_Rifkhan" href="https://instagram.com/Rashard_Rifkhan" />
        ...
        <div className="lg:col-span-7">
          <form onSubmit={onSubmit} className="border border-border/60 bg-card p-7 md:p-10 space-y-5">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              ...
              <Field name="interest" label="Primary Interest" placeholder="e.g. Royal Blue Sapphires" />
            </div>
            <div>
              <label className="block text-[10px] tracking-[0.3em] uppercase text-muted-foreground mb-2">Message</label>
              <textarea name="message" rows={5} required maxLength={1000} className="w-full bg-input/60 border border-border focus:border-gold/60 focus:outline-none px-4 py-3 text-foreground text-sm resize-none transition-colors" placeholder="Tell us about the stones or jewelry you're seeking, your travel plans, or any questions." />
            </div>
            <div className="flex flex-wrap items-center justify-between gap-4 pt-2">
              <p className="text-[10px] tracking-[0.2em] uppercase text-muted-foreground">Confidential · No spam · Direct reply</p>
              <button type="submit" className="btn-gold">{sent ? "Sent ✓ Send Again" : "Send Inquiry"}</button>
            </div>
          </form>
...
function ContactRow({ label, value, href }: { label: string; value: string; href?: string }) {
  const Inner = (
    <>
      ...
      <div className="mt-1 text-lg text-foreground group-hover:text-gold transition-colors">{value}</div>
    </>
  );
  return href ? (
    <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel="noreferrer" className="group block border-b border-border/50 pb-4">{Inner}</a>
  ) : (
    ...
    <footer className="border-t border-border/60 bg-ink">
        ...
        <div className="md:col-span-2">
          ...
          <div className="mt-6 flex gap-3">
            {["NGJA Registered", "Chinfort Member"].map((b) => (
              <span key={b} className="text-[10px] tracking-[0.25em] uppercase border border-gold/40 text-gold-soft px-3 py-1.5">{b}</span>
            ))}
          </div>
        </div>
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {[["Collection","#collection"],["Services","#services"],["Experience","#experience"],["Contact","#contact"]].map(([l,h])=>(
              <li key={h}><a href={h} className="text-foreground/70 hover:text-gold transition-colors">{l}</a></li>
            ))}
          </ul>
        </div>
        <div>
          ...
          <ul className="space-y-2 text-sm text-foreground/70">
            <li><a href="tel:+94774579076" className="hover:text-gold">+94 774 579 076</a></li>
            <li><a href={WHATSAPP} target="_blank" rel="noreferrer" className="hover:text-gold">WhatsApp</a></li>
            <li><a href="https://instagram.com/Rashard_Rifkhan" target="_blank" rel="noreferrer" className="hover:text-gold">@Rashard_Rifkhan</a></li>
      ...
      <div className="border-t border-border/60">
        <div className="mx-auto max-w-7xl px-5 md:px-10 py-6 flex flex-wrap items-center justify-between gap-3 text-[10px] tracking-[0.25em] uppercase text-muted-foreground">
          <span>© {new Date().getFullYear()} Aurora Gems · Beruwala · Colombo · Bangkok</span>
